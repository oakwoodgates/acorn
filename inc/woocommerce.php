 ...
 * @return void
 */
function acorn_woocommerce_scripts() {
	wp_enqueue_style( 'acorn-woocommerce-style', get_template_directory_uri() . '/woocommerce.css' );
	...
	wp_add_inline_style( 'acorn-woocommerce-style', $inline_font );
}
add_action( 'wp_enqueue_scripts', 'acorn_woocommerce_scripts' );
 ...
 *
	<?php
		if ( function_exists( 'acorn_woocommerce_header_cart' ) ) {
			acorn_woocommerce_header_cart();
		}
	?>
 */

if ( ! function_exists( 'acorn_woocommerce_cart_link_fragment' ) ) {
	/**
	 * Cart Fragments.
	 *
	 ...
	 */
	function acorn_woocommerce_cart_link_fragment( $fragments ) {
		ob_start();
		acorn_woocommerce_cart_link();
		$fragments['a.cart-contents'] = ob_get_clean();

		return $fragments;
	}
}
add_filter( 'woocommerce_add_to_cart_fragments', 'acorn_woocommerce_cart_link_fragment' );

if ( ! function_exists( 'acorn_woocommerce_cart_link' ) ) {
	/**
	 * Cart Link.
	 *
	 ...
	 * @return void
	 */
	function acorn_woocommerce_cart_link() {
		?>
		<a class="cart-contents" href="<?php echo esc_url( wc_get_cart_url() ); ?>" title="<?php esc_attr_e( 'View your shopping cart', 'acorn' ); ?>">
			<?php
			$item_count_text = sprintf(
				/* translators: number of items in the mini cart. */
				_n( '%d item', '%d items', WC()->cart->get_cart_contents_count(), 'acorn' ),
...
if ( ! function_exists( 'acorn_woocommerce_header_cart' ) ) {
	 ...
	 * @return void
	 */
	function acorn_woocommerce_header_cart() {
		if ( is_cart() ) {
		...
		<ul id="site-header-cart" class="site-header-cart">
			<li class="<?php echo esc_attr( $class ); ?>">
				<?php acorn_woocommerce_cart_link(); ?>
			</li>
			<li>
				<?php
				$instance = array(
